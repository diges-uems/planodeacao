 ...
 *   Isso existe porque, até 2026-08-27, o doGet() retornava TODOS os dados de TODOS os cursos sem qualquer
 *   autenticação (bastava saber a URL do Web App, que é pública e visível no bundle JS do frontend) — e várias
 *   ações de escrita (delete/update/liberar_edicao/save_deadlines/etc.) também não validavam quem estava chamando.
 *
 ...
 * - Além do "Liberado", agora o token do coordenador precisa corresponder ao mesmo courseId do registro que ele
 *   está editando/excluindo (evita que um coordenador autenticado edite registros de outro curso).
 *
 * SISTEMA DE E-MAILS:
...
// Sessão do token: 12 horas (o frontend não persiste o token em localStorage, então o coordenador
// já refaz o login a cada refresh; isso só evita que um token vaze e continue válido para sempre).
var TOKEN_VALIDADE_MS = 12 * 60 * 60 * 1000;

/**
 * =========================================================================
 * AUTENTICAÇÃO POR TOKEN (HMAC-SHA256)
 * =========================================================================
 * O token carrega role/courseId/courseName/expiração, assinados com uma chave secreta
 * guardada em ScriptProperties (nunca exposta ao frontend). O frontend não consegue forjar
 * nem alterar um token — qualquer adulteração invalida a assinatura.
 */

/**
 * Recupera (ou gera, na primeira execução) a chave secreta usada para assinar os tokens.
 * Fica guardada em ScriptProperties, nunca é enviada ao cliente.
 */
function getSecret() {
...
/**
 * Gera um token assinado para a sessão de login (role + courseId + courseName + expiração).
 */
function gerarToken(role, courseId, courseName) {
  var payload = JSON.stringify({
    role: role,
    courseId: courseId || '',
    courseName: courseName || '',
    exp: Date.now() + TOKEN_VALIDADE_MS
  });
  var payloadB64 = Utilities.base64EncodeWebSafe(payload);
  var assinatura = Utilities.base64EncodeWebSafe(
    Utilities.computeHmacSha256Signature(payloadB64, getSecret())
  );
  return payloadB64 + '.' + assinatura;
}

/**
 * Valida um token recebido do frontend. Retorna as claims ({role, courseId, courseName, exp})
 * se válido e não expirado, ou null caso contrário (assinatura inválida, formato errado, expirado).
 */
function validarToken(token) {
  ...
  var payloadB64 = partes[0];
  var assinaturaRecebida = partes[1];

  var assinaturaEsperada = Utilities.base64EncodeWebSafe(
    Utilities.computeHmacSha256Signature(payloadB64, getSecret())
  );
  ...
  var claims;
  try {
    claims = JSON.parse(Utilities.newBlob(Utilities.base64DecodeWebSafe(payloadB64)).getDataAsString());
  } catch (e) {
    return null;
  }
...
/**
 * Resposta padrão de "não autorizado", usada sempre que um token está ausente,
 * inválido/expirado, ou a role da claim não tem permissão para a ação solicitada.
 */
 ...
 * Utilizado para a funcionalidade de buscar prazos internos (CONFIG_PRAZOS) e para
 * buscar todos os dados de fragilidades. Ambas exigem um token válido (?token=...).
 */
function doGet(e) {
 ...
 *
 * TODA ação, exceto "login", exige um token válido em data.token (obtido no login).
 * Ações restritas à PROE (save_deadlines, liberar_edicao, cobrar_atraso/atualizacao,
 * test_email) exigem role === 'reitoria'. Ações de coordenador (envio de lote,
 * update/delete/add_acompanhamento, register_course_email) exigem que o courseId do
 * token corresponda ao curso sendo alterado, a menos que a role seja 'reitoria'.
 */
function doPost(e) {
  // CORS Headers
  ...
  try {
      ...
      for (var i = 1; i < cData.length; i++) {
        if (cData[i][0] === inputHash) {
          return ContentService.createTextOutput(JSON.stringify({
              ...
              token: gerarToken('coordenador', cData[i][1], cData[i][2])
            })).setMimeType(ContentService.MimeType.JSON);
        }
      }
    ...
    // (envio de lote de fragilidades pelo coordenador), o token não vem em data.token
    // (arrays JSON não têm essa propriedade) — o frontend embute o token em cada item,
    // então usamos o token do primeiro item do lote.
    var tokenRecebido = Array.isArray(data) ? (data.length > 0 ? data[0].token : null) : data.token;
    var claims = validarToken(tokenRecebido);
    if (!claims) return respostaNaoAutorizado();
    ...
    if (data.action === "register_course_email") {
      ...
      if (claims.role !== 'reitoria' && String(claims.courseId) !== courseId) {
    ...
    // =====================================================================
    // COBRANÇAS E-MAIL (exclusivo PROE)
    // =====================================================================
    if (data.action === "cobrar_atraso" || data.action === "cobrar_atualizacao") {
      if (claims.role !== 'reitoria') return respostaNaoAutorizado();
      var curso = data.curso || 'Não informado';
      var fragilidade = data.fragilidade || 'Não informada';
      var responsavel = data.responsavel || 'Não informado';
      var id = data.id || 'Não informado';

      var assunto = "";
      var introducao = "";

      if (data.action === "cobrar_atraso") {
          assunto = "[PROE/UEMS] Aviso de Prazo Expirado - Plano de Ação";
          introducao = "Identificamos que uma ação sob responsabilidade do seu curso encontra-se com a <b>data final expirada</b> no sistema do Plano de Ação.";
      } else {
          assunto = "[PROE/UEMS] Atualização de Status - Plano de Ação";
          introducao = "Solicitamos a atualização do status da ação abaixo, que se encontra em andamento no sistema do Plano de Ação.";
      }

      var detalhesHTML = "<div class='detail-row'><span class='detail-label sans'>Curso</span><span class='detail-value sans'>" + curso + "</span></div>" +
                         "<div class='detail-row'><span class='detail-label sans'>ID da Ação</span><span class='detail-value sans'>" + id + "</span></div>" +
                         "<div class='detail-row'><span class='detail-label sans'>Fragilidade/Ação</span><span class='detail-value sans'>" + fragilidade + "</span></div>" +
                         "<div class='detail-row'><span class='detail-label sans'>Responsável</span><span class='detail-value sans'>" + responsavel + "</span></div>";

      var finalizacaoHTML = "<p style='margin-top: 20px; font-size: 14px; color: #334155; font-family: sans-serif;'>" +
                            "Por favor, acesse o sistema para atualizar o status e registrar os acompanhamentos necessários." +
                            "</p>";

      var cursosSheet = ss.getSheetByName('CURSOS');
      var emailDestino = "enade@uems.br";
      if (cursosSheet) {
          var cData = cursosSheet.getDataRange().getValues();
          for (var i = 1; i < cData.length; i++) {
              if (String(cData[i][2]).trim() === String(curso).trim()) {
                  var foundEmail = cData[i][3];
                  if (foundEmail && foundEmail.toString().trim() !== "") {
                      emailDestino = foundEmail.toString().trim();
                  }
                  break;
              }
          }
    ...
    // =====================================================================
    // LIBERAR EDIÇÃO (exclusivo PROE)
    // =====================================================================
    if (data.action === "liberar_edicao") {
      if (claims.role !== 'reitoria') return respostaNaoAutorizado();
      var cursosSheet = ss.getSheetByName('CURSOS');
      if (!cursosSheet) return ContentService.createTextOutput(JSON.stringify({ success: false, message: 'Aba CURSOS não encontrada.' })).setMimeType(ContentService.MimeType.JSON);
    ...
    if (Array.isArray(data)) {
      ...
      if (claims.role === 'coordenador') {
        for (var vi = 0; vi < data.length; vi++) {
          if (String(data[vi].codigoCurso) !== String(claims.courseId)) {
            return respostaNaoAutorizado();
          }
        }
      }
      ...
      data.forEach(function(item) {
        ...
        lotes[ano][curso].push([
          ...
          item.acompanhamentos ? formatarAcompanhamentosParaTexto(item.acompanhamentos) : ''
        ]);
      });
      ...
      for (var ano in lotes) {
        ...
        for (var curso in lotes[ano]) {
          ...
          var codigoCurso = rowsToInsert[0][2];
    ...
    if (data.action === "update" || data.action === "delete" || data.action === "add_acompanhamento") {
      ...
      // Código e nome do curso são lidos da própria linha encontrada na planilha (colunas C e D),
      ...
      if (data.action === "update" || data.action === "delete") {
        ...
        if (cursosSheet) {
          var cData = cursosSheet.getDataRange().getValues();
          for (var i = 1; i < cData.length; i++) {
            if (String(cData[i][1]) === String(codigoCursoDaLinha)) {
      ...
      if (rowIndex !== -1) {
        if (data.action === "delete") {
          ...
          var acompanhamentos = [];
          if (acompStr) {
            try { acompanhamentos = parseAcompanhamentosTextoParaArray(acompStr); } catch(err) { acompanhamentos = []; }
          }
          if (!Array.isArray(acompanhamentos)) acompanhamentos = [];
          acompanhamentos.push(data.acompanhamento);
          sheet.getRange(rowIndex, 16).setValue(formatarAcompanhamentosParaTexto(acompanhamentos));

          if (data.acompanhamento.novoPrazo) {
            var headers = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0];
            var idxPrazo = headers.indexOf("Prazo") !== -1 ? headers.indexOf("Prazo") + 1 : (headers.indexOf("Data Final") !== -1 ? headers.indexOf("Data Final") + 1 : 0);
            if (idxPrazo > 0) {
              sheet.getRange(rowIndex, idxPrazo).setNumberFormat("@").setValue(data.acompanhamento.novoPrazo);
            }
          }
...
/**
 * Função de checagem única: garante que a aba CURSOS tenha o cabeçalho completo
 * (hash | courseId | courseName | Email | Liberado), mesmo em planilhas antigas
 * que já existiam antes desses dois campos serem adicionados ao sistema.
 * Roda manualmente, uma vez, pelo editor do Apps Script. É seguro rodar mais de uma vez.
 */
function garantirCabecalhoCursos() {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var sheet = ss.getSheetByName('CURSOS');
  if (!sheet) {
    Logger.log("Aba CURSOS não encontrada.");
    return;
  }

  var headerAtual = sheet.getRange(1, 1, 1, 5).getValues()[0];
  var headerEsperado = ['hash', 'courseId', 'courseName', 'Email', 'Liberado'];
  var precisaCorrigir = false;

  for (var i = 0; i < headerEsperado.length; i++) {
    if (headerAtual[i] !== headerEsperado[i]) {
  ...
  if (precisaCorrigir) {
    sheet.getRange(1, 1, 1, 5).setValues([headerEsperado]);
    sheet.getRange(1, 1, 1, 5).setFontWeight("bold").setBackground("#f1f5f9");
    Logger.log("Cabeçalho da aba CURSOS corrigido para: " + headerEsperado.join(" | "));
  } else {
    Logger.log("Cabeçalho da aba CURSOS já está correto.");
  }

  // Preenche "NÃO" como padrão explícito na coluna Liberado para linhas antigas que estão vazias
  // (funcionalmente já equivale a bloqueado, mas deixa o estado visível/explícito na planilha).
  var data = sheet.getDataRange().getValues();
  for (var r = 1; r < data.length; r++) {
    if (data[r][1] && (data[r][4] === "" || data[r][4] === undefined)) {
...
function formatarAcompanhamentosParaTexto(acompanhamentos) {
  if (!acompanhamentos || acompanhamentos.length === 0) return "";
  var blocos = [];
  for (var i = 0; i < acompanhamentos.length; i++) {
    var ac = acompanhamentos[i];
    var dataStr = "";
    try {
      var d = new Date(ac.dataRegistro);
      if (!isNaN(d.getTime())) {
         dataStr = Utilities.formatDate(d, "GMT-04:00", "dd/MM/yyyy HH:mm");
      } else {
         dataStr = ac.dataRegistro;
      }
    } catch(e) { dataStr = ac.dataRegistro; }

    var linhas = [];
    linhas.push("[" + dataStr + "] " + ac.status);
    linhas.push("Por: " + ac.registradoPor);
    if (ac.novoPrazo) {
      var novoPrazoFmt = ac.novoPrazo;
      if (/^\d{4}-\d{2}-\d{2}$/.test(ac.novoPrazo)) {
         novoPrazoFmt = ac.novoPrazo.split('-').reverse().join('/');
      }
      linhas.push("Novo Prazo: " + novoPrazoFmt);
    }
    linhas.push("Obs: " + ac.descricao);
...
function parseAcompanhamentosTextoParaArray(texto) {
  ...
  for (var i = 0; i < blocos.length; i++) {
    var bloco = blocos[i].trim();
    if (!bloco) continue;

    var ac = { dataRegistro: "", status: "", registradoPor: "", descricao: "", novoPrazo: "" };
    var linhas = bloco.split("\n");
    var primeiraLinha = linhas[0] || "";
    var matchHeader = primeiraLinha.match(/^\[(.*?)\]\s*(.*)$/);
    if (matchHeader) {
      ac.dataRegistro = matchHeader[1];
      ac.status = matchHeader[2].trim();
    } else {
      ac.status = primeiraLinha.trim();
    }
    ...
    for (var j = 1; j < linhas.length; j++) {
      var l = linhas[j].trim();
      if (l.indexOf("Por: ") === 0) {
        ac.registradoPor = l.substring(5).trim();
      } else if (l.indexOf("Novo Prazo: ") === 0) {
        ac.novoPrazo = l.substring(12).trim();
        if (/^\d{2}\/\d{2}\/\d{4}$/.test(ac.novoPrazo)) {
           ac.novoPrazo = ac.novoPrazo.split('/').reverse().join('-');
        }
      } else if (l.indexOf("Obs: ") === 0) {
        obsLinhas.push(l.substring(5).trim());
      } else {
        obsLinhas.push(l);
      }
    }
    ac.descricao = obsLinhas.join("\n");
    arr.push(ac);
  }
  return arr;
}
